...
export default function AnatomyScene({atlas,state,onSelect,onProgress,onError}:Props){
 ...
 useEffect(()=>{
  ...
  renderer.domElement.setAttribute('aria-label','Interactive bacterial ultrastructure. Drag to orbit, pinch or scroll to zoom, and tap a structure to inspect it.');
  ...
  let loaded=0;
  const pickMat=new T.MeshBasicMaterial({side:T.DoubleSide});materials.push(pickMat);
  const loadChunk=async(ci:number)=>{
   ...
   atlas.parts.forEach((p,i)=>{
    ...
    g.boundingBox=bounds[i].clone();g.computeBoundingSphere();const pick=new T.Mesh(g,pickMat);pick.matrixAutoUpdate=false;pickers[i]=pick;geometries.push(g);
  ...
  const fit=(view:string,extent=0)=>{
   ...
   controls.target.set(extent>.1&&el.clientWidth>767?-packingWidth*.04:0,extent>.1?0.16:(mobile?.85:.68),0);camera.position.copy(controls.target).addScaledVector(direction,distance);controls.update();dirty=true;
  };
  ...
  const animate=()=>{
   ...
   if(changed||moving||lastExtent<0){
    ...
    atlas.parts.forEach((p,i)=>{
     ...
     else {const t=(amount-.45)/.55,group=SYSTEMS.findIndex(sys=>sys.id===p.system),angle=group/SYSTEMS.length*Math.PI*2;dx=T.MathUtils.lerp(Math.sin(angle)*.48,destination.x-c.x,t);dy=T.MathUtils.lerp((c.y-.85)*.28,destination.y-c.y,t);dz=T.MathUtils.lerp(Math.cos(angle)*.48,0,t);}
   ...
   controls.enableRotate=amount<.8;controls.mouseButtons.LEFT=amount<.8?T.MOUSE.ROTATE:T.MOUSE.PAN;controls.touches.ONE=amount<.8?T.TOUCH.ROTATE:T.TOUCH.PAN;ground.visible=platform.visible=ring.visible=innerRing.visible=amount<.5&&!s.isolate;markers.visible=false;controls.autoRotate=s.rotate&&!s.isolate&&amount<.4;controls.autoRotateSpeed=.65;controls.update();if(controls.autoRotate)dirty=true;
